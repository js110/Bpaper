...
\label{tab:robust}\input{robust_table}
\end{table}
...
First, nominal BSP relies on one declared belief model. R-BSP turns this limitation into an explicit finite ambiguity-set guarantee by intersecting member-wise constraints, and the post-hoc stress test shows the expected removal of local-cap violations when the informed attacker's exact model is included. This does not solve model uncertainty in general. The nine-model grid was selected after nominal failures were known, is conservative even when the nominal model is correct, and offers no theorem for attackers outside the set. A deployment would need a defensible model-set construction procedure---for example from development/validation uncertainty---followed by evaluation that was not used to choose that set.
