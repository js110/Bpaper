...
\begin{document}
\begin{frontmatter}
\title{Truthful Reports and Informative Silence:\\A Bayesian Participation Filter for Sequential Mobile Crowdsensing}
\author[aff1]{Lili He}
...
\begin{abstract}
Keeping precise coordinates on a mobile device does not eliminate the location information contained in successful region-constrained sensing tasks. Silence can also be informative without establishing that a user is outside a region. We study this binary channel for linkable accounts under uncertain delivery, participation, and timely completion. A branch-safe participation (BSP) filter limits the maximum posterior mass after either output. We derive its maximal scalar gate and a model-conditional sequential bound. A software evaluation combines synthetic mobility and replay of 67 GeoLife users excluded from model fitting. At opportunity retention 0.5, exploratory policy envelopes selected on test results give static-scenario MAP hit-rate reductions of 8.8 and 10.6 percentage points against task-channel adaptations of PML and PRIVIC, both published in 2024. Pointwise cluster-bootstrap intervals for this selection procedure exclude zero, conditional on fixed training; other scenarios show no consistent advantage over both methods. Two diagnostics qualify the utility and privacy interpretation: completed tasks mainly cover large regions, and a development-fitted, no-output location predictor attains \PopulationPriorHit\% replay accuracy, exceeding the reported uniform-prior inference accuracy under BSP. A better-informed attacker can also exceed the client's cap. These results support a conditional participation mechanism for the declared channel, while limiting claims about practical attacker accuracy, fine-region task service, and general superiority.
\end{abstract}
\begin{keyword}
...
\end{keyword}
\end{frontmatter}
\noindent\textbf{Draft status.} This manuscript contains executed experiments and reproducible analyses. The author, affiliation, corresponding-author, and funding details below were supplied for this draft; author approval and verification of the remaining declarations and citations remain pending. No submission or external peer review has occurred.
...
The study provides three concrete outputs. First, it specifies the observation boundary and derives a closed-form, maximal feasible gate for the binary channel. The accompanying sequential bound states exactly when mobility prediction preserves the cap. Second, it compares the gate with task-channel instantiations of two 2024 mechanisms, PRIVIC~\cite{privic2024} and pointwise maximal leakage (PML) optimization~\cite{pml2024}, alongside random participation, periodic limits, aligned-region admission, a success-only ablation, and a KL branch filter~\cite{bayesian}. Third, it reports the limits of these findings: several matched-utility comparisons are inconclusive, a weaker ablation can achieve a lower empirical hit rate, and more accurate attacker knowledge invalidates the client's interpretation of its own bound.
...
\begin{figure}[tbp]
\centering\includegraphics[width=\linewidth]{figures/system_architecture.png}
...
\begin{figure}[tbp]
\centering\includegraphics[width=\linewidth]{figures/bsp_workflow.png}
...
The implementation uses half-open rectangles. It rejects duplicate observations and validates task identifiers, observation identifiers, and expired tasks. True locations are consulted only by the response simulator and evaluation metrics. Archived evaluation logs contain ground truth for auditing, but the attack receives only public tasks, outcomes, model parameters, and the gate determined by public history.
...
A later exploratory extension adds a KL branch filter at $\kappa=0.25,0.5,1,2$ nats. This adaptation of Bayesian privacy~\cite{bayesian} selects the largest scalar $q$ satisfying $D_{\mathrm{KL}}(b^y\|b)\le\kappa$ for both positive-probability branches. The positive divergence is $-\log\Pr_b(S)$; silence divergence reduces to a binary region partition and is solved with 30 bisection steps. It limits each update relative to its current prior, not cumulative session KL or an absolute posterior maximum. It is not a reproduction of the published economic mechanism.
...
\label{eq:pml}
\end{equation}
If $M=\sum_{i\in S}b_i<e^{-\epsilon}$, a nonzero report is infeasible: summing the report constraint over $S$ gives $m\le e^\epsilon M m$. If $e^{-\epsilon}\le M<1$, the outside-state silence constraint and the availability bound give $m\le\min(\alpha M,1-e^{-\epsilon})$. Constant likelihood $l_i=\alpha q$ inside $S$, with $q=\min\{1,(1-e^{-\epsilon})/(\alpha M)\}$, attains both bounds. When $M=1$, $q=1$ is feasible; when $M=0$, the gate does not affect the output. Thus this specialization is exactly optimal for current completion even among location-dependent gates. We verify it against an independent general linear program over 100 random priors. This is our binary-channel instantiation of the published constraints, not a reproduction of the paper's general-output vertex-enumeration experiment. The tested $\epsilon$ values are $0.025,0.05,0.1,0.25,0.5,1,2,4,8$ nats per update.
...
\label{tab:prior-diagnostic}\input{prior_diagnostic_table}
\end{table}

\begin{figure}[tbp]
\centering\includegraphics[width=\linewidth]{attacks.pdf}
...
\begin{figure}[tbp]
\centering\includegraphics[width=\linewidth]{tradeoff.pdf}
...
\label{tab:matched}\input{matched_table}
\end{table}
...
\begin{figure}[tbp]
\centering\includegraphics[width=\linewidth]{recent_tradeoff.pdf}
...
\label{tab:recent}\input{recent_matched_table}
\end{table}
...
\begin{figure}[tbp]
\centering\includegraphics[width=\linewidth]{calibration.pdf}
...
\begin{figure}[tbp]
\centering\includegraphics[width=\linewidth]{sensitivity.pdf}
...
\label{tab:informed}\input{informed_table}
\end{table}
...
First, BSP relies on one declared belief model. The informed-attacker results show that stronger protection would require accounting for multiple plausible priors and transitions, or changing the output channel. Intersecting constraints across candidate models is a possible extension, but its cost and utility are not evaluated here. The current cap should not be advertised as protection against arbitrary auxiliary knowledge.
...
Twenty-one tests cover posterior branches, gate bounds and maximality, KL feasibility, transition conservation, unbiased ties, boundaries, duplicates, expired observations, missing minutes, split separation, paired normal tasks, numerical replay, author-code BA/IBU conformance, PML optimality against a general linear program, vector-valued observation channels, and policy-envelope agreement with a mixture linear program. The corrected pilot, interrupted KL run, and initial shorter PRIVIC training are retained separately and excluded from the final comparisons. Completed runs contain no recorded simulation failures.

The raw GeoLife archive is separately sourced; the study does not claim new collection, anonymization, or unrestricted redistribution. The artifact is local and has not been publicly deposited. The supplied author and funding details require author verification; conflicts, contributions, the funders' roles, and any required ethics or data-use determinations remain for responsible authors to supply. AI assistance was used for research organization, code, analysis, drafting, and the two conceptual schematics in Figs.~\ref{fig:architecture} and~\ref{fig:workflow}. Their prompts, original raster files, revision history, and agent visual checks are archived; experimental plots are generated from measured simulation logs. Final disclosure and scientific verification require author review under the selected venue's policy.

\section{Conclusion}
Sequential regional task outputs can reveal location despite local storage of precise coordinates. Probabilistic silence must be included in that analysis. BSP provides a maximal scalar gate for both posterior branches and a sequential cap under explicit model and transition conditions. Exploratory comparisons favor it in some settings, while retained participation concentrates on large-region tasks. Replay accuracy under a uniform-prior inference model is not a practical privacy bound: a development-fitted predictor can perform better without any outputs. A more accurately informed attacker can also exceed the client-side cap. The evidence supports a conditional mechanism and reproducible diagnostics, for applications that expose linkable truthful task outcomes. The comparison with encrypted, padded allocation protocols identifies a different observation boundary, while broader novelty and practical benefit require further evidence.
...
\label{tab:task-area}\input{task_area_table}
\end{table}
...
\begin{figure}[htbp]
\centering\includegraphics[width=.84\linewidth]{recent_commute_zoom.pdf}
