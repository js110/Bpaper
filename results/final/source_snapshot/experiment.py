...
def simulate(condition,seed,user,path,rects,masks,slots,side,log):
    ...
    for t in range(slots):
        ...
        elif attack=='adaptive': idx=choose_probe(belief.b,masks,rects,alpha_model,method,param,t)
        ...
        reported=bool(eligible and available and ext[t,3]<q)
        ...
        metrics=posterior_metrics(belief.b,truth,side)
        ...
        if condition['scenario']=='geolife':
            ties=np.flatnonzero(np.isclose(belief.b,belief.b.max(),rtol=1e-10,atol=1e-14))
        ...
        if log:
            rows.append(dict(slot=t,task_id=task.task_id,rectangle=rects[idx],legitimate=is_legit,
                             reported=reported,q=q,alpha_model=alpha_model,prior_peak=float(bprior.max()),
                             belief=belief.b.tolist(),evaluator_truth=truth,eligible=eligible,available=available,**metrics))
